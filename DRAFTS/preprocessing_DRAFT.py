        ...
        if train_folder_pattern.match(folder) is not None:
            ...
            for tf in range(time_frames):
                ...
                for k, v in d.items():
                    v = tensorizer(v)
                    save_image(v,
                               os.path.join(out_dir, "merged", fns[i], f'{folder.split("-", 1)[-1]}{str(tf)}-{k}.png'))

                    bri_cont = bri_cont_transform(v)
                    save_image(bri_cont,
                               os.path.join(out_dir, "bri_cont", fns[i],
                                            f'{folder.split("-", 1)[-1]}{str(tf)}-{k}-bri-cont.png'))

                    inverted = inverter_transform(bri_cont)
                    save_image(inverted,
                               os.path.join(out_dir, "invert", fns[i],
                                            f'{folder.split("-", 1)[-1]}{str(tf)}-{k}-invert.png'))

                    # inverted = np.invert(v)
                    # # inverted = cv2.bitwise_not(v)
                    # # inverted = ~v
                    temporal_frames.append(inverted)

            temporal_frames = torch.stack(tuple(temporal_frames), axis=0)
            write_video(os.path.join(out_dir, "invert_video", fns[i], f"{folder.split('-', 1)[-1]}.mp4"), temporal_frames, fps=2)

            out = cv2.VideoWriter(os.path.join(out_dir, "invert_video", fns[i],
                                    f"{folder.split('-', 1)[-1]}.mp4"), cv2.VideoWriter_fourcc(*'DIVX'), 2, (1024, 1024))
            for i in range(time_frames):
                out.write(temporal_frames[i])
